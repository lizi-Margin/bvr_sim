...
bool FGFDMExec::ReadFileHeader(Element* el)
{
  ...
  std::array<std::string, 4> element_names = {"description", "author", "filecreationdate", "version"};
  for (auto& element_name: element_names) {
    if (el->FindElement(element_name)) {
      if (debug_lvl > 0) {
        FGLogging log(Log, LogLevel::DEBUG);
        log << "  " << element_name << ": " << el->FindElement(element_name)->GetDataLine() << endl;
      }
    } else {
      FGLogging log(Log, LogLevel::ERROR);
      log << "  " << element_name << ": " << "Not found" << endl;
      is_valid = false;
    }
  }
