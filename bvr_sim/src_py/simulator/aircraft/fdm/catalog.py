# Third Party Code
# From LAG (Light Air Game):
# https://github.com/liuqh16/LAG

import re
import math
from enum import Enum
from collections import namedtuple
from typing import Callable, Optional, Any, TYPE_CHECKING, List, Dict
...
def in_range_deg(angle: float) -> float:
    angle = angle % 360
    if angle > 180:
        angle -= 360
    return angle
...
class JsbsimCatalog(Property, Enum):
    ...
    accelerations_pdot_rad_sec2 = Property(
        "accelerations/pdot-rad_sec2", "rad/s²", -(8 / 180) * math.pi, (8 / 180) * math.pi, access="R"
    )
    accelerations_qdot_rad_sec2 = Property(
        "accelerations/qdot-rad_sec2", "rad/s²", -(8 / 180) * math.pi, (8 / 180) * math.pi, access="R"
    )
    accelerations_rdot_rad_sec2 = Property(
        "accelerations/rdot-rad_sec2", "rad/s²", -(8 / 180) * math.pi, (8 / 180) * math.pi, access="R"
    )
    accelerations_vdot_ft_sec2 = Property("accelerations/vdot-ft_sec2", "ft/s²", -4.0, 4.0, access="R")
    accelerations_wdot_ft_sec2 = Property("accelerations/wdot-ft_sec2", "ft/s²", -4.0, 4.0, access="R")
    accelerations_udot_ft_sec2 = Property("accelerations/udot-ft_sec2", "ft/s²", -4.0, 4.0, access="R")
    accelerations_a_pilot_x_ft_sec2 = Property(
        "accelerations/a-pilot-x-ft_sec2", "pilot body x-axis acceleration [ft/s²]", access="R"
    )
    accelerations_a_pilot_y_ft_sec2 = Property(
        "accelerations/a-pilot-y-ft_sec2", "pilot body y-axis acceleration [ft/s²]", access="R"
    )
    accelerations_a_pilot_z_ft_sec2 = Property(
        "accelerations/a-pilot-z-ft_sec2", "pilot body z-axis acceleration [ft/s²]", access="R"
    )
    ...
    @staticmethod
    def update_equal_engine_props(sim: 'JSBSimFDM', prop: Property) -> None:
    ...
    def update_equal_throttle_pos(sim: 'JSBSimFDM') -> None:
        JsbsimCatalog.update_equal_engine_props(sim, JsbsimCatalog.fcs_throttle_pos_norm.value)

    def update_equal_mixture_pos(sim: 'JSBSimFDM') -> None:
        JsbsimCatalog.update_equal_engine_props(sim, JsbsimCatalog.fcs_mixture_pos_norm.value)

    def update_equal_feather_pos(sim: 'JSBSimFDM') -> None:
        JsbsimCatalog.update_equal_engine_props(sim, JsbsimCatalog.fcs_feather_pos_norm.value)

    def update_equal_advance_pos(sim: 'JSBSimFDM') -> None:
        JsbsimCatalog.update_equal_engine_props(sim, JsbsimCatalog.fcs_advance_pos_norm.value)
    ...
    def update_equal_throttle_cmd(sim: 'JSBSimFDM') -> None:
        JsbsimCatalog.update_equal_engine_props(sim, JsbsimCatalog.fcs_throttle_cmd_norm.value)

    def update_equal_mixture_cmd(sim: 'JSBSimFDM') -> None:
        JsbsimCatalog.update_equal_engine_props(sim, JsbsimCatalog.fcs_mixture_cmd_norm.value)

    def update_equal_advance_cmd(sim: 'JSBSimFDM') -> None:
        JsbsimCatalog.update_equal_engine_props(sim, JsbsimCatalog.fcs_advance_cmd_norm.value)

    def update_equal_feather_cmd(sim: 'JSBSimFDM') -> None:
        JsbsimCatalog.update_equal_engine_props(sim, JsbsimCatalog.fcs_feather_cmd_norm.value)

    @staticmethod
    def update_equal_brake_props(sim: 'JSBSimFDM') -> None:
        value = sim.get_property_value(JsbsimCatalog.fcs_center_brake_cmd_norm.value)
        sim.jsbsim_exec.set_property_value(JsbsimCatalog.fcs_left_brake_cmd_norm.value.name_jsbsim, value)
        sim.jsbsim_exec.set_property_value(JsbsimCatalog.fcs_right_brake_cmd_norm.value.name_jsbsim, value)

    def update_equal_brake_cmd(sim: 'JSBSimFDM') -> None:
    ...
    position_h_sl_m = Property("position/h-sl-m", "altitude above mean sea level [m]", -500, 26000, access="R")
    velocities_v_north_mps = Property("velocities/v-north-mps", "velocity true north [m/s]", -700, 700, access="R")
    velocities_v_east_mps = Property("velocities/v-east-mps", "velocity east [m/s]", -700, 700, access="R")
    velocities_v_down_mps = Property("velocities/v-down-mps", "velocity downwards [m/s]", -700, 700, access="R")


class ExtraCatalog(Property, Enum):

    def _update_position_h_sl_m(sim: 'JSBSimFDM') -> None:
        sim.set_property_value(
            ExtraCatalog.position_h_sl_m.value,
            sim.get_property_value(JsbsimCatalog.position_h_sl_ft.value) * 0.3048
        )

    position_h_sl_m = Property(
        "position/h-sl-m", "altitude above mean sea level [m]", -500, 26000, access="R",
        update=_update_position_h_sl_m
    )

    def _update_velocities_v_north_mps(sim: 'JSBSimFDM') -> None:
        sim.set_property_value(
            ExtraCatalog.velocities_v_north_mps.value,
            sim.get_property_value(JsbsimCatalog.velocities_v_north_fps.value) * 0.3048
        )

    velocities_v_north_mps = Property(
        "velocities/v-north-mps", "velocity true north [m/s]", -700, 700, access="R",
        update=_update_velocities_v_north_mps
    )

    def _update_velocities_v_east_mps(sim: 'JSBSimFDM') -> None:
        sim.set_property_value(
            ExtraCatalog.velocities_v_east_mps.value,
            sim.get_property_value(JsbsimCatalog.velocities_v_east_fps.value) * 0.3048
        )

    velocities_v_east_mps = Property(
        "velocities/v-east-mps", "velocity east [m/s]", -700, 700, access="R",
        update=_update_velocities_v_east_mps
    )

    def _update_velocities_v_down_mps(sim: 'JSBSimFDM') -> None:
        sim.set_property_value(
            ExtraCatalog.velocities_v_down_mps.value,
            sim.get_property_value(JsbsimCatalog.velocities_v_down_fps.value) * 0.3048
        )

    velocities_v_down_mps = Property(
        "velocities/v-down-mps", "velocity downwards [m/s]", -700, 700, access="R",
        update=_update_velocities_v_down_mps
    )

    def _update_velocities_vc_mps(sim: 'JSBSimFDM') -> None:
        sim.set_property_value(
            ExtraCatalog.velocities_vc_mps.value,
            sim.get_property_value(JsbsimCatalog.velocities_vc_fps.value) * 0.3048
        )

    velocities_vc_mps = Property(
        "velocities/vc-mps", "airspeed in knots [m/s]", 0, 1400, access="R",
        update=_update_velocities_vc_mps
    )

    def _update_velocities_u_mps(sim: 'JSBSimFDM') -> None:
        sim.set_property_value(
            ExtraCatalog.velocities_u_mps.value,
            sim.get_property_value(JsbsimCatalog.velocities_u_fps.value) * 0.3048
        )

    velocities_u_mps = Property(
        "velocities/u-mps", "body frame x-axis velocity [m/s]", -700, 700, access="R",
        update=_update_velocities_u_mps
    )

    def _update_velocities_v_mps(sim: 'JSBSimFDM') -> None:
        sim.set_property_value(
            ExtraCatalog.velocities_v_mps.value,
            sim.get_property_value(JsbsimCatalog.velocities_v_fps.value) * 0.3048
        )

    velocities_v_mps = Property(
        "velocities/v-mps", "body frame y-axis velocity [m/s]", -700, 700, access="R",
        update=_update_velocities_v_mps
    )

    def _update_velocities_w_mps(sim: 'JSBSimFDM') -> None:
        sim.set_property_value(
            ExtraCatalog.velocities_w_mps.value,
            sim.get_property_value(JsbsimCatalog.velocities_w_fps.value) * 0.3048
        )

    velocities_w_mps = Property(
        "velocities/w-mps", "body frame z-axis velocity [m/s]", -700, 700, access="R",
        update=_update_velocities_w_mps
    )

    def _update_delta_altitude(sim: 'JSBSimFDM') -> None:
        value = (sim.get_property_value(ExtraCatalog.target_altitude_ft.value) -
                 sim.get_property_value(JsbsimCatalog.position_h_sl_ft.value)) * 0.3048
        sim.set_property_value(ExtraCatalog.delta_altitude.value, value)
    ...
    delta_altitude = Property(
        ...
        40000,
        access="R",
        update=_update_delta_altitude,
    )

    def _update_delta_heading(sim: 'JSBSimFDM') -> None:
        value = in_range_deg(
            sim.get_property_value(ExtraCatalog.target_heading_deg.value) -
            sim.get_property_value(JsbsimCatalog.attitude_psi_deg.value)
        )
        sim.set_property_value(ExtraCatalog.delta_heading.value, value)
    ...
    delta_heading = Property(
        ...
        180,
        access="R",
        update=_update_delta_heading,
    )

    def _update_delta_velocities(sim: 'JSBSimFDM') -> None:
        value = (sim.get_property_value(ExtraCatalog.target_velocities_u_mps.value) -
                 sim.get_property_value(ExtraCatalog.velocities_u_mps.value))
        sim.set_property_value(ExtraCatalog.delta_velocities_u.value, value)
    ...
    delta_velocities_u = Property(
        ...
        1400,
        access="R",
        update=_update_delta_velocities,
    )

    @staticmethod
    def update_property_incr(sim: 'JSBSimFDM', discrete_prop: Property, prop: Property, incr_prop: Property) -> None:
    ...
    elevator_cmd_dir = Property(
        ...
        spaces=Discrete,
        access="W",
        update=_update_elevator_cmd_dir,
    )
    incr_elevator = Property("fcs/incr-elevator", "incrementation elevator", 0, 1)

    def _update_rudder_cmd_dir(sim: 'JSBSimFDM') -> None:
        ExtraCatalog.update_property_incr(
            sim,
            ExtraCatalog.rudder_cmd_dir.value,
            JsbsimCatalog.fcs_rudder_cmd_norm.value,
            ExtraCatalog.incr_rudder.value
        )
    ...
    rudder_cmd_dir = Property(
        ...
        spaces=Discrete,
        access="W",
        update=_update_rudder_cmd_dir,
    )
    incr_rudder = Property("fcs/incr-rudder", "incrementation rudder", 0, 1)

    def _update_detect_extreme_state(sim: 'JSBSimFDM') -> None:
        extreme_velocity = sim.get_property_value(JsbsimCatalog.velocities_eci_velocity_mag_fps.value) >= 1e10
        extreme_rotation = (
            norm(
                [
                    sim.get_property_value(JsbsimCatalog.velocities_p_rad_sec.value),
                    sim.get_property_value(JsbsimCatalog.velocities_q_rad_sec.value),
                    sim.get_property_value(JsbsimCatalog.velocities_r_rad_sec.value),
                ]
            ) >= 1000
        )
        extreme_altitude = sim.get_property_value(JsbsimCatalog.position_h_sl_ft.value) >= 1e10
        extreme_acceleration = (
            max(
                [
                    abs(sim.get_property_value(JsbsimCatalog.accelerations_n_pilot_x_norm.value)),
                    abs(sim.get_property_value(JsbsimCatalog.accelerations_n_pilot_y_norm.value)),
                    abs(sim.get_property_value(JsbsimCatalog.accelerations_n_pilot_z_norm.value)),
                ]
            ) > 1e1
        )
        sim.set_property_value(
    ...
    target_altitude_ft = Property(
        ...
        JsbsimCatalog.position_h_sl_ft.value.min,
        JsbsimCatalog.position_h_sl_ft.value.max,
    )
    target_heading_deg = Property(
        "tc/target-heading-deg",
        "target heading [deg]",
        JsbsimCatalog.attitude_psi_deg.value.min,
        JsbsimCatalog.attitude_psi_deg.value.max,
    )
...
class MixedCatalog(dict):
    ...
    def add_jsbsim_props(self, jsbsim_props: List[str]) -> None:
