...
class AIM120C(Missile):
    def __init__(
        ...
        self._Rc = feet_to_meters(1000.0)
        self._mach_min = 0.8
    ...
    def step(self) -> None:
        ...
        if distance < self._Rc and self.target is not None and self.target.is_alive:
            ...
            if isinstance(self.target, Aircraft):
                ...
                raise ValueError(f"AIM120C target must be an Aircraft or GroundUnit, got {type(self.target)}")
    ...
    def _guidance(self):
        ...
        # loft logic
        if self._t < 4.0:
            if np.degrees(self.posture[1]) < min(meters_to_nm(Rxyz), 10.0):
                deps = max(deps, 0.1)
